...
tic
output_t=5000;            % How often to output/save data
tf=31557600*5000;         % Run Duration (s) this will be final time of 
                          % run
...
melt_hold=zeros(size(Geo_Flux,1),size(Geo_Flux,2),tf/(31557600*output_t));
h_hold=zeros(size(Geo_Flux,1),size(Geo_Flux,2),tf/(31557600*output_t));
...
%         j=Htot(jj);
        ...
        rho_v,melt_hold,reservoir,h_hold,res,output_t);
        %% end of loop
%     end
toc
